  ...
  private async createRoomSensors(
    ...
    if (existingAccessory) {
      ...
      if (await this.registerDevice(device)) {
        ...
        this.debugLog(
          `${sensorAccessory.accessoryAttribute.type} uuid: ${sensorAccessory.accessoryAttribute.type}-${sensorAccessory.accessoryId}-RoomSensor, (${existingAccessory.UUID})`,
        )
      } else {
        this.unregisterPlatformAccessories(existingAccessory)
      }
  ...
  private async createRoomSensorThermostat(
    ...
    if (existingAccessory) {
      ...
      if (await this.registerDevice(device)) {
        ...
        this.debugLog(
          `${sensorAccessory.accessoryAttribute.type} Thermostat uuid: ${sensorAccessory.accessoryAttribute.type}-${sensorAccessory.accessoryId}-RoomSensorThermostat, (${existingAccessory.UUID})`,
        )
      } else {
        this.unregisterPlatformAccessories(existingAccessory)
      }
